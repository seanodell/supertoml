...
fn process_value_with_jinja(value: &toml::Value, context: &HashMap<String, toml::Value>) -> Result<toml::Value, SuperTomlError> {
    ...
    match value {
        toml::Value::String(s) => {
            if s.contains("{{") || s.contains("{%") {
                let template = env.template_from_str(s).map_err(|e| {
                ...
                let rendered = template.render(&context_jinja).map_err(|e| {
                    SuperTomlError::PluginError {
                        plugin_name: "templating".to_string(),
